...
import { ProductReview } from '@voguish/module-catalog';
import { Suspense, lazy, useState } from 'react';
const ProductReviewItem = lazy(() => import('../../ProductReviewItem'));
...
export default function ReviewsList({ reviews }: { reviews: ProductReview[] }) {
  ...
  return (
    ...
    >
      <Suspense>
        {!isValidArray(reviews) ? (
          <Typography variant="body2">For now no reviews added.</Typography>
        ) : (
          <>
            {reviews.map(
              (review, index) =>
